...
template<typename T>
 __global__ void rotary_embed(const T *in, const float *cos_sin, const int* pos, T *out,
                              const size_t bs, const size_t hnum, const size_t len, const size_t dims) {
...
int kr_rotary_embed<float>( const float *in, const float *cos_sin, const int* pos, float *out,
                            const size_t bs, const size_t hnum, const size_t len, const size_t dims, hipStream_t stream) {
    dim3 block_size(256);
