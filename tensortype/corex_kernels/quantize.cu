...
template <>
int kr_dequantize_pq<__half>(const void *tab, const uint8_t *idx, __half *out, const size_t items_, int S, cudaStream_t stream) {
    dim3 block_size(256);
...
template <>
int kr_quantize_q4<float>(const float *in, void *out, const size_t items, cudaStream_t stream) {
...
template <>
int kr_quantize_q4<__half>(const __half *in, void *out, const size_t items, cudaStream_t stream) {
    int blk_num = items / Q4_BLOCK_SIZE;
...
template <>
int kr_dequantize_q4<__half>(const void *in, __half *out, const size_t items, cudaStream_t stream) {
...
template <>
int kr_quantize_q8<__half>(const __half *input, void *out, int feature_num, int feature_size, cudaStream_t stream) {
    dim3 block_size(256);
...
template <>
int kr_quantize_q8<float>(const float *input, void *out, int feature_num, int feature_size, cudaStream_t stream) {
    dim3 block_size(256);
	dim3 num_of_blocks((feature_num + block_size.x - 1) / block_size.x);
...
template <>
int kr_dequantize_q8<__half>(const void *input, __half *out, int feature_num, int feature_size, cudaStream_t stream) {
    dim3 block_size(128);
