...
template<typename T>
__global__ void easy_top3(const T *logits, int *out, const int batch, const int vocab_size,const float temp, const float randx) {
...
template<>
int kr_easy_top3<__half>(const __half* logits, int *out,
...
template<>
int kr_easy_top1<__half>(const __half* logits, int *out,
                         const int batch, const int vocab_size, hipStream_t stream) {
    dim3 block_size(256);
	dim3 num_of_blocks(batch);
