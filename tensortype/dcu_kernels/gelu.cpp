...
template <>
int kr_gelu<float>(const float* src, float* target, int nElementNumber, hipStream_t stream) {
    dim3 block_size(256);
...
template <>
int kr_gelu<__half>(const __half* src, __half* target, int nElementNumber, hipStream_t stream) {
    dim3 block_size(256);
	dim3 num_of_blocks((nElementNumber + block_size.x - 1) / block_size.x);
