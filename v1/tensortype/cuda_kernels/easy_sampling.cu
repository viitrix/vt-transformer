...
__global__ void easy_top3_kernel(const T *logits, int *out, 
                                const size_t batch, const size_t vocab_size,const float temp, const float randx) {
    const int K = 3;
    const int DIM = 256;
...
// ================================================
template<typename T>
__global__ void kr_easy_top1(const T *logits, int *out, const size_t batch, const size_t vocab_size) {
    const int DIM = 256;
    assert(blockDim.x == DIM);
    ...
    vt::cuda::blockReduce<cuda::ReduceType::kTop, 1>(vals);
    top_idx = vals[1];
...
int kr_sampling_top1<__half>(const __half* logits, int *out,
                         const size_t batch, const size_t vocab_size, cudaStream_t stream) {
    dim3 block_size(256);
	dim3 num_of_blocks(batch);
