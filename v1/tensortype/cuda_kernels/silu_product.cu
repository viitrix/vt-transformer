...
template <>
int kr_silu_product<float>(const float* in_act, const float* in, float* out, size_t nElementNumber, cudaStream_t stream) {
    dim3 block_size(256);
...
template <>
int kr_silu_product<__half>(const __half* in_act, const __half* in, __half* out, size_t nElementNumber, cudaStream_t stream) {
    dim3 block_size(256);
	dim3 num_of_blocks((nElementNumber + block_size.x - 1) / block_size.x);
